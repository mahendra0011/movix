...
function AuthPage() {
  ...
  useEffect(() => {
    ...
    navigate(routeForRole(auth.user), { replace: true });
  }, [auth.hydrated, auth.user, authReady, navigate]);

  useEffect(() => {
    if (!search || !search.token || !search.user) return;
    try {
      const user = JSON.parse(decodeURIComponent(search.user));
      dispatch(setCredentials({ user, token: search.token }));
    } catch {
      setNotice({ tone: "error", text: "Google sign-in failed." });
    }
  }, [search, dispatch]);
  ...
  const completeSignIn = async (result) => {
    ...
    await navigate(routeForRole(result.user), { replace: true });
  };
    ...
    <div className="relative min-h-[calc(100vh-190px)] overflow-hidden">
      <img
        src={normalizeMovieImageUrl(
          authMovie.backdrop || authMovie.poster,
          authMovie.title,
          "backdrop",
        )}
        alt={authMovie.title}
        ...
        <section className="hidden lg:block">
          ...
          <div className="mt-8 max-w-xl rounded-lg border border-border/60 bg-background/50 p-4 backdrop-blur">
            <div className="flex items-center gap-3">
              <img
                src={normalizeMovieImageUrl(authMovie.poster, authMovie.title, "poster")}
                alt={authMovie.title}
            ...
            {!otpStep && mode === "login" && (
              <>
                ...
                <a
                  ...
                  className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-md border border-border/60 bg-background text-sm font-medium text-foreground transition-colors hover:bg-muted"
                >
            ...
            {!otpStep && mode === "login" && (
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() =>
                    setForm((f) => ({ ...f, email: "admin@demo.com", password: "demo@1234" }))
                  }
                  className="rounded-md border border-border/60 bg-background px-3 py-1.5 text-xs font-medium text-muted-foreground hover:border-primary hover:text-primary transition-colors"
                >
                  Demo Admin
                </button>
                <button
                  type="button"
                  onClick={() =>
                    setForm((f) => ({ ...f, email: "owner@demo.com", password: "demo@1234" }))
                  }
                  className="rounded-md border border-border/60 bg-background px-3 py-1.5 text-xs font-medium text-muted-foreground hover:border-primary hover:text-primary transition-colors"
                >
                  Demo Owner
                ...
                <button
                  type="button"
                  onClick={() =>
                    setForm((f) => ({ ...f, email: "user@demo.com", password: "demo@1234" }))
                  }
                  className="rounded-md border border-border/60 bg-background px-3 py-1.5 text-xs font-medium text-muted-foreground hover:border-primary hover:text-primary transition-colors"
                >
                  Demo User
